...
test("consumeWeakMatch returns the most recent fresh entry for a key", () => {
  ...
  buf.push({ queryText: "어디 살아", weakKeyScores: new Map([["k1", 0.9]]) });
  clock = 1010;
  buf.push({ queryText: "사는곳", weakKeyScores: new Map([["k1", 0.95]]) });
...
test("consumeWeakMatch removes the key so it cannot fire twice", () => {
  ...
  buf.push({ queryText: "q", weakKeyScores: new Map([["k1", 0.9]]) });
...
test("TTL boundary: entry at cutoff edge is still fresh, expires one second later", () => {
  ...
  buf2.push({ queryText: "q", weakKeyScores: new Map([["k1", 0.9]]) });
  clock = 301;
...
test("capacity evicts oldest entries", () => {
  ...
  buf.push({ queryText: "a", weakKeyScores: new Map([["k1", 0.9]]) });
  buf.push({ queryText: "b", weakKeyScores: new Map([["k2", 0.9]]) });
  buf.push({ queryText: "c", weakKeyScores: new Map([["k3", 0.9]]) });
